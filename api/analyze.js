...
module.exports=async function handler(req,res){
 if(req.method!=='GET')return send(res,405,{error:'Use GET'});const {sm}=tokens(req);if(!sm)return send(res,400,{error:'A análise avançada requer o token Sportmonks'});const id=String(req.query?.id||'');if(!/^\d+$/.test(id))return send(res,400,{error:'fixture id inválido'});
 ...
 await Promise.all([
  ...
  (async()=>{if(!fixture.has_odds&&!fixture.has_premium_odds)return;try{const {data}=await fetchJson(smUrl(`/odds/pre-match/fixtures/${id}`,sm,{include:'market;bookmaker'}),{},12000);oddsRaw=data?.data||[]}catch(e){errors.push(`Odds: ${e.message}`)}})()
 ]);
 const homeForm=formStats(hf,home.id,10),awayForm=formStats(af,away.id,10),h2hResult=h2hStats(h2h,home.id,away.id,10),odds=oddsTriplet(oddsRaw,home.name,away.name),statsCount=Array.isArray(fixture.statistics)?fixture.statistics.length:0,live=[2,3,4,6,7,9,22].includes(Number(fixture.state_id));
 const engine=probabilityEngine({odds,homeForm,awayForm,h2h:h2hResult,live,statsCount});
 return send(res,200,{fixture:{id:String(fixture.id),home:{id:String(home.id),name:home.name,logo:home.image_path||''},away:{id:String(away.id),name:away.name,logo:away.image_path||''},league:fixture.league?.name||'',start:fixture.starting_at,state:fixture.state?.short_name||fixture.state?.name||'',live},...engine,odds:odds?{home:odds[0],draw:odds[1],away:odds[2]}:null,form:{home:homeForm,away:awayForm},h2h:h2hResult,statsCount,errors});
};
